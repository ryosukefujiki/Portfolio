...
<div class='TheTransition TheHiveTransition'>
  <div class='TheTransition_Background TheNoren_Background' ref='background'></div>
  <div class='TheTransition_Background TheNoren_Background' ref='background'></div>
    <div class='TheTransition_Background TheNoren_Background' ref='background'>
        <p class='TheTransition_Category' ref='category'>Software</p>
        <p class='TheTransition_Title' ref='title'>のれんさま</p>
    </div>
    <div class='TheTransition_Background TheNoren_Background' ref='background'></div>
  <div class='TheTransition_Background TheNoren_Background' ref='background'></div>
</div>
  ...
  methods: { // アニメーションの宣言はここ
    enter () { // `entered`が`true`になったとき発火
      requestAnimationFrame(() => {
        TweenMax.staggerTo(".TheNoren_Background", 0.55, {
          x: '0%',
          ease: Expo.easeOut
        },0.04)
      })
    ...
    async set (){
      requestAnimationFrame(() => {
        TweenMax.staggerTo(".TheNoren_Background", 0, {
          opacity: '0%',
          ease: Expo.easeOut
        },0.04)
      })
