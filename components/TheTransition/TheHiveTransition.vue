...
<div class='TheTransition TheHiveTransition'>
  <div class='TheTransition_Background TheHive_Background' ref='background'></div>
  <div class='TheTransition_Background TheHive_Background' ref='background'></div>
    <div class='TheTransition_Background TheHive_Background' ref='background'>
       <p class='TheTransition_Category' ref='category'>Logo Design</p>
        <p class='TheTransition_Title' ref='title'>Hive</p>
    </div>
  <div class='TheTransition_Background TheHive_Background' ref='background'></div>
  <div class='TheTransition_Background TheHive_Background' ref='background'></div>
</div>
  ...
  methods: { // アニメーションの宣言はここ
    enter () { // `entered`が`true`になったとき発火
      requestAnimationFrame(() => {
        TweenMax.staggerTo(".TheHive_Background", 0.55, {
          x: '0%',
          ease: Expo.easeOut
        },0.04)
      })
    ...
    leave () { // `entered`が`false`になったとき発火
      ...
      this.$store.commit('hive/fadeIn')
    },
    async set (){
      requestAnimationFrame(() => {
        TweenMax.staggerTo(".TheHive_Background", 0, {
          opacity: '0%',
          ease: Expo.easeOut
        },0.04)
      })
